...
EVALUATION_CONFIGS = {
    "keywords": {
        "gold_folder": "Gold_Standards/gold_standard_keywords",
        "root_folder": "Results/Results_with_Wikidata_QIDs_Keywords",
        "gold_entities_key": "keywords",
        "pred_entities_key": "keywords",
    },
    "named_entities": {
        "gold_folder": "Gold_Standards/goldStandard_Named_Entites",
        "root_folder": "Results/Results_with_Wikidata_QIDs_Named_Entities",
        "gold_entities_key": "entities",
...
def get_evaluation_config(evaluation_type):
    """
    Resolve folders and entity keys for the selected evaluation type.
    """
    try:
        return EVALUATION_CONFIGS[evaluation_type]
    except KeyError:
        valid_types = ", ".join(sorted(EVALUATION_CONFIGS))
        raise ValueError(
            f"evaluationType non valido: '{evaluation_type}'. Usa uno tra: {valid_types}"
        )


def get_entities(item, preferred_key):
    """
    Return the entity list using the configured key, with a small fallback.
    """
    entities = item.get(preferred_key)
    if entities is not None:
        return entities

    for fallback_key in ("keywords", "entities", "named_entities"):
        entities = item.get(fallback_key)
        if entities is not None:
            return entities
...
# Metriche globali per l'keyword linking
    ...
    for filename, gold_content in gold_data.items():
        ...
        for gold_entities, pred_entities in zip(gold_content, predicted_content):
            gold_entities_list = get_entities(gold_entities, gold_entities_key)
            pred_entities_list = get_entities(pred_entities, pred_entities_key)

            gold_labels = [
                (entity["Wikipedia_label"], extract_wikidata_id(entity["Wikidata_ID"], nullTP))
                for entity in gold_entities_list
            ]
            pred_entities_processed = [
                (entity["originalKey"], entity["Wikidata_ID"]) for entity in pred_entities_list
            ]
    ...
    for dirpath, dirnames, filenames in os.walk(root_folder):
        ...
        try:
            metrics = calculate_metrics(
                ...
                predicted_data,
                jaccard_threshold,
                gold_entities_key=gold_entities_key,
                pred_entities_key=pred_entities_key,
            )
            (
...
def plot_all_metrics_trend(gold_folder, root_folder):
    ...
    for model_name in sorted(os.listdir(root_folder)):
        ...
        for t in thresholds:
            ...
            precision, recall, f1, _, _,_ = calculate_metrics(
                gold_data,
                predicted_data,
                t,
                gold_entities_key=gold_entities_key,
    ...
    for model_name in sorted(os.listdir(root_folder)):
        ...
        try:
            ...
            (_prec, _rec, _f1,
             _tp_per_file,
             false_positive_per_file,
             _fn_per_file) = calculate_metrics(
                gold_data,
                predicted_data,
                jaccard_threshold,
                nullTP=nullTP,
...
# parameters (change evaluationType to switch dataset automatically)
evaluationType = "keywords"  # or "named_entities" 

config = get_evaluation_config(evaluationType)

metric_type = "keyword linking"
gold_folder = config["gold_folder"]

root_folder = config["root_folder"]
gold_entities_key = config["gold_entities_key"]
pred_entities_key = config["pred_entities_key"]
